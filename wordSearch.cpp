...
public:
        ...
        if (board.size() > 0) {
            ...
            for(int i = 0; i<board.size(); i++){
                for(int j = 0; j<board[0].size(); j++){ // 这么低级的错误，i、j没有赋初值
                    cout << "outer board char: " << board[i][j] << endl;
                    if(board[i][j] == word[0]){
                        cout << "inner board char: " << board[i][j] << endl;
                        bool flag_existed = false;
                        searchWord(board, i, j, word, 1, visited, flag_existed);
                        if(flag_existed)    return true;
                        else{   // 对下一个位置起始的字符发起搜索前，要把visited全部置为0
                            for(int m = 0; m<board.size(); m++){
                                for(int n = 0; n<board[0].size(); n++){
                                    if(visited[m][n] != 0)
                                        visited[m][n] = 0;
    ...
    void searchWord(vector< vector<char> >& board, int i, int j, string word, int k, int **visited, bool & flag_existed){   // 最后一个参数表示，word中的第k个位置
                                                                                        // 要比较word[k]和board[i][j]上下左右的四个字符是否相等
        visited[i][j] = 1;  // 传入的board[i][j]已经访问过，要递归地访问它上下左右的邻居

        if(k == word.length())  // 如果满足此条件，说明 word 从第0个到第 len(word)-1 个全部匹配上，即找到该单词
            flag_existed = true;

        int x_neighbor[] = {0,0,-1,1};
        int y_neighbor[] = {-1,1,0,0};

        for(int dir=0; dir<4; dir++){
            int R = i + x_neighbor[dir];
            int C = j + y_neighbor[dir];

            if(isSafe(board, R, C, visited)){
                if(board[R][C] == word[k]){
                    cout << "recursion char: " << word[k] << endl;
                    searchWord(board, R, C, word, k+1, visited, flag_existed);
                }
            }
        }
